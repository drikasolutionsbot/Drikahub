...
import { Key, Webhook, CheckCircle2, AlertCircle, ExternalLink, Copy, Zap, Loader2, Info } from "lucide-react";
...
const PushinPayIntegrationTab = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (data) {
        setConfigId(data.id);
        const d = data as any;
        if (d.pushinpay_api_key) setApiKey(d.pushinpay_api_key);
  ...
  const handleToggleActive = async (checked: boolean) => {
    ...
    try {
      const { error } = await supabase
        .from("landing_config")
        .update({ pushinpay_active: checked, updated_at: new Date().toISOString() } as any)
        .eq("id", configId);
      if (error) throw error;
      setIsConnected(checked);
      toast.success(checked ? "PushinPay ativado!" : "PushinPay desativado!");
    } catch {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success("Configurações da PushinPay salvas!");
    } catch (err: any) {
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            {isConnected ? (
              <>
                ...
                <div>
                  <p className="font-medium text-emerald-500">PushinPay ativo</p>
                  <p className="text-xs text-muted-foreground">Cobranças de assinatura ativas via PIX PushinPay</p>
                </div>
              </>
            ) : (
              <>
                ...
                <div>
                  <p className="font-medium text-amber-500">PushinPay inativo</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="bg-card border-border">
          <CardHeader>
            ...
            <CardDescription>Configure seu token de API da PushinPay</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <p className="text-xs text-muted-foreground">
              ...
              <a href="https://pushinpay.com.br" target="_blank" rel="noreferrer" className="text-primary hover:underline inline-flex items-center gap-1">
                painel PushinPay <ExternalLink className="h-3 w-3" />
              </a>
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-card border-border">
          <CardHeader>
            ...
            <CardDescription>Configure este URL no painel da PushinPay para receber confirmações PIX</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Cole esta URL nas configurações de webhook da PushinPay
              </p>
            </div>
            ...
            <div className="rounded-lg border border-border bg-muted/30 p-3">
              <div className="flex items-start gap-2">
                ...
                <p className="text-xs text-muted-foreground">
                  A PushinPay é mais simples de configurar — basta o token de API. Não requer certificado mTLS.
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <Card className="bg-card border-border">
        <CardHeader>
          ...
          <CardDescription>O botão "Assinar Pro" na landing page gerará um PIX automaticamente via PushinPay</CardDescription>
        </CardHeader>
        <CardContent>
